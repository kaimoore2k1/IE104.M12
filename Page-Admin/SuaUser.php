...
<body>
    ...
    <main class="main">
        ...
        <div class="col-md-8">
            <h2 class=" text-center">Sửa User</h2>
            <div class="col-md-8 div--css">
                <form action="" class="form--css" method="post">
                    <?php while($row = $kq->fetch_assoc()) {?>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="Last_Name" class="control-label">Họ</label>
                            <input name="Last_Name" value="<?php echo $row["Last_Name"] ?>" class="form-control" required/>
    
                        </div>
                        <div class="form-group col-md-6">
                            <label for="First_Name" class="control-label">Tên</label>
                            <input name="First_Name" value="<?php echo $row["First_Name"] ?>" class="form-control" required/>
    
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="User_Name" class="control-label">User Name</label>
                        <input name="User_Name" value="<?php echo $row["User_Name"] ?>" class="form-control" required/>
                    </div>
                    <div class="form-group">
                ...
                <?php 
                    if(isset($_POST["submit"])) { 
                        ...
                        $sql = "UPDATE user SET Last_Name = '$Last_Name',First_Name = '$First_Name', User_Name = '$User_Name', User_Password = '$User_Password' WHERE User_Id = '$id'";
                            
                        $conn->query($sql);
                        echo "<script> window.location.href = '../Page-Admin/QuanLyUser.php' </script>";
                    }
